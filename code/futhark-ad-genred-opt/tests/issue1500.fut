let secant_method (f:f32 -> f32) (a:f32) (b:f32) (tol:f32) =
    (loop (x_1, x_2, f_1, f_2) = (a, b, f a, f b) while (f32.abs (x_1-x_2) > tol) do
        let x_0 = x_1 - f_1 * (x_1 - x_2) / (f_1 - f_2)
        let f_0 = f x_0
        in (x_0, x_1, f_0, f_1)).0

let minimum_D (stress:f32 -> f32) =
    let f D = stress D
...
let turning   = running true  3 2 0
let main = turning.1
